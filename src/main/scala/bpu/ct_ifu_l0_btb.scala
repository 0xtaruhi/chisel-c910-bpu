...
withClockAndReset(io.forever_cpuclk.asClock,(!io.cpurst_b).asAsyncReset){
    ...
    val l0_btb_rd_flop_temp = RegNext(l0_btb_rd,0.U(1.W))
...
entry_rd_hit.zip(entry_tag).zip(entry_vld).foreach(x => {x._1._1 := (l0_btb_rd_tag === x._1._2).asBool &&
                                                  x._2.asBool && !io.pcgen_l0_btb_chgflw_mask})
...
entry_bypass_hit.zip(entry_rd_hit).zip(entry_hit).foreach(x=>{x._2 := x._1._1.asBool || x._1._2.asBool})



val entry_hit_flop = Wire(UInt(ENTRY_SIZE.W))
withClockAndReset(io.forever_cpuclk.asClock, (!io.cpurst_b).asAsyncReset){
    val entry_hit_flop_temp = RegInit(0.U(ENTRY_SIZE.W))
    when(l0_btb_rd){
...
l0_btb_ras_pc := MuxCase(io.ras_l0_btb_pc,Array(io.ras_l0_btb_ras_push -> io.ras_l0_btb_push_pc,
 ...
 val ras_pc = Wire(UInt((PC_WIDTH-1).W))
  ...
  {
    val ras_pc_temp = RegInit(0.U((PC_WIDTH-1).W))
    ras_pc := ras_pc_temp
...
entry_hit_target :=   Mux(entry_hit_ras,ras_pc,Cat(io.pcgen_l0_btb_if_pc(PC_WIDTH-2,(ENTRY_TARGET)),
  entry_hit_pcs.asUInt))
...
val entry_fifo_temp = RegInit(1.U((ENTRY_SIZE).W))
...
l0_btb_update_entry.zip(io.addrgen_l0_btb_update_entry.asBools).zip(
  ...
  when(io.addrgen_l0_btb_update_vld === 1.U){
     ...
     l0_btb_update_data    := 0.U((PC_WIDTH-3).W)
  ...
  withClockAndReset(l0_btb_inv_reg_upd_clk,(!io.cpurst_b).asAsyncReset){
    val l0_btb_entry_inv_temp = RegInit(0.U(1.W))
